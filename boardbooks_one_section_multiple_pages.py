...
if __name__ == "__main__":
    link = "https://go.boarddocs.com/mabe/bcps/Board.nsf/goto?open&id=D87NSZ6166CB"
    meeting_date = "Dec 03, 2024"
    year_drop = "2024"
    #title = 'Professional Services Contracts'
    output_file = '2200540_EAST BATON ROUGE PARISH_10-17-24_Professional Services Contracts.pdf'
    ...
    #     element_meeting_date = WebDriverWait(driver, 10).until(
    ...
    #     print(f"Clicked on meeting with {meeting_date}.")
        ...
        # Locate the specific <span> by its text content
        category_name = "NEW BUSINESS - CONTRACT AWARDS"
        try:
            # Wait up to 20 seconds for the element to appear
            category_span = WebDriverWait(driver, 20).until(
        ...
        # Navigate to the parent <dt> tag
        category_dt = category_span.find_element(By.XPATH, './..')

        # Find the adjacent <dd> tag containing the links
        links_dd = category_dt.find_element(By.XPATH, './following-sibling::dd')

        # Iterate through the <li> tags within the <dd>
        list_items = links_dd.find_elements(By.TAG_NAME, 'li')

        # Extract titles and links
        for item in list_items:
            title = item.find_element(By.CLASS_NAME, 'title').text
            unique_id = item.get_attribute('unique')  # Example: Extract the 'unique' attribute
            print(f"Title: {title}, Unique ID: {unique_id}")
            unique_element = WebDriverWait(driver, 10).until(
            EC.element_to_be_clickable((By.XPATH, f"//*[@unique='{unique_id}']")))
            unique_element.click()
            print(f"Clicked element with unique ID: {unique_id}")
            #time.sleep(30)
            # Find the element with an ID containing 'attachment-public-'
            # element = WebDriverWait(driver, 20).until(
            #     EC.presence_of_element_located((By.XPATH, "//*[contains(@id, 'attachment-public-')]")))
            ...
            # parent_div = wait.until(EC.presence_of_element_located((By.ID, element_id)))  # Wait for the parent div to be present
            ...
            file_links = driver.find_elements(By.CSS_SELECTOR,"a.public-file" )
